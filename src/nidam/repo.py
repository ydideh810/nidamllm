...
from nidam.common import INTERACTIVE, REPO_DIR, VERBOSE_LEVEL, RepoInfo, load_config, output, save_config

UPDATE_INTERVAL = datetime.timedelta(days=3)
TEST_REPO = os.getenv('NIDAM_TEST_REPO', None)  # for testing


app = NidamTyper(help='manage repos')
...
def _complete_alias(repo_name: str):
    from nidam.model import list_jile

    for jile in list_jile(repo_name=repo_name):
        alias = jile.labels.get('nidam_alias', '').strip()
        if alias:
            for a in alias.split(','):
                with open(jile.path.parent / a, 'w') as f:
                    f.write(jile.version)
...
def ensure_repo_updated():
    ...
    if not last_update_file.exists():
        if INTERACTIVE.get():
            ...
            output(
                'The repo cache is never updated, please run `nidam repo update` to fetch the latest model list',
                style='red',
            )
            raise typer.Exit(1)
    ...
    if datetime.datetime.now() - last_update > UPDATE_INTERVAL:
        if INTERACTIVE.get():
            ...
            output(
                'The repo cache is outdated, please run `nidam repo update` to fetch the latest model list',
                style='yellow',
            )
...
def parse_repo_url(repo_url: str, repo_name: typing.Optional[str] = None) -> RepoInfo:
    ...
    >>> parse_repo_url('https://github.com/jileml/jilevllm@main')
    ('github.com', 'jileml', 'jilevllm', 'main')

    >>> parse_repo_url('https://github.com/jileml/jilevllm.git@main')
    ('github.com', 'jileml', 'jilevllm', 'main')

    >>> parse_repo_url('https://github.com/jileml/jilevllm')
    ('github.com', 'jileml', 'jilevllm', 'main')

    >>> parse_repo_url('git@github.com:jileml/nidam-models.git')
    ('github.com', 'jileml', 'nidam-models', 'main')
    """
